...
$(function () {
	...
	if ($('.datatable-responsive').length) {
		...
		$('.datatable-responsive').DataTable({
			"processing": true,
			"serverSide": true,
			"ajax": base_url + 'admin/addon/alldata/',
...
$(document).ready(function () {
	$(document).on("click", '.change_status', function () {
		...
		bootbox.confirm("Are you sure? You want to " + $status + " this addon.", function (result) {


			if (result) {
				$.ajax({
					url: base_url + 'admin/addon/update/' + $id + '/',
					type: 'POST',
	...
	$("table").on("click", ".delete", function(){
		...
		bootbox.confirm("Are you sure? You want to delete this row?", function (result) {
			...
			if (result) {
				$.ajax({
					url: base_url + 'admin/addon/delete/' + $id + '/',
					type: 'POST',
					data: {},
					success: function (response) {
