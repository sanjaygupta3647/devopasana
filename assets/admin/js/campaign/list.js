...
$(function () {
	...
	if ($('.datatable-responsive').length) {
		...
		$('.datatable-responsive').DataTable({
			"processing": true,
			"serverSide": true,
			"ajax": base_url + 'admin/campaign/allcampaigns/' + $param,
...
$(document).ready(function () {
	$(document).on("click", '.change_status', function () {
		...
		bootbox.confirm("Are you sure? You want to " + $status + " this campaign.", function (result) {


			if (result) {
				$.ajax({
					url: base_url + 'admin/campaign/update/' + $id + '/',
					type: 'POST',
	...
	$("#filter").click(function () {
		var url = base_url + 'admin/campaign/';
		$status = $("#status").val();
		if ($status) {
			url += '?status=' + $status
		}
